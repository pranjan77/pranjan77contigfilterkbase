package pranjan77contigfilterkbase::pranjan77contigfilterkbaseClient;
...
pranjan77contigfilterkbase::pranjan77contigfilterkbaseClient

=head1 DESCRIPTION


A KBase module: pranjan77contigfilterkbase
...
sub new
{
    ...
    my $self = {
	client => pranjan77contigfilterkbase::pranjan77contigfilterkbaseClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a pranjan77contigfilterkbase.FilterContigsParams
$return is a pranjan77contigfilterkbase.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a pranjan77contigfilterkbase.workspace_name
	contigset_id has a value which is a pranjan77contigfilterkbase.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is a pranjan77contigfilterkbase.ws_contigset_id
...
# Authentication: required
	...
	method => "pranjan77contigfilterkbase.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
<pre>
$params is a pranjan77contigfilterkbase.FilterContigsMaxParams
$return is a pranjan77contigfilterkbase.FilterContigsResults
...
# Authentication: required
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
	method => "pranjan77contigfilterkbase.filter_contigs_max",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "pranjan77contigfilterkbase.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for pranjan77contigfilterkbase::pranjan77contigfilterkbaseClient\n";
    }
    if ($sMajor == 0) {
        warn "pranjan77contigfilterkbase::pranjan77contigfilterkbaseClient version is $svr_version. API subject to change.\n";
    }
}
...
package pranjan77contigfilterkbase::pranjan77contigfilterkbaseClient::RpcClient;
